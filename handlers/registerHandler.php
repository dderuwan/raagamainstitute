...
if (isset($_POST["regNext"])) {
    ...
    $sql = "INSERT INTO Instructors (id, Status, FirstName, LastName, Address, Address2, Country, City, ZipCode, Phone, Email, Password, EducationL, DegreeP, Experience, ExperienceLevel, pdfPath) VALUES (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $status = "Pending";
    
    if ($stmt = mysqli_prepare($connection, $sql)) {
        mysqli_stmt_bind_param($stmt, "ssssssssssssssss" , $status, $firstName, $lastName, $address, $address2, $country, $city, $zipcode, $phone, $email, $hashed_password, $educationL, $degreeP, $Experience, $ExperienceLevel, $resume);
